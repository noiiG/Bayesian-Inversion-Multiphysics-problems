function [KK_phase,H_n1_new,F_phase,PI_vec,PJ_vec]=phase_field_resi_M(EL, W,...
    H_se_n,phase_n1_i,u_n1_i,phase_n1,gamma,infoPHF)
...
for ele = 1 : numelem
    ...
    for kk = 1 : size(W,1)      % Looping on Gauss point
        ...
        strain=Bfem*disp_u;
        gamma_gp=gamma(ele,1).data(kk,1).s;
        phase_gp=(N' * phase);
        phasen_gp=(N' * phase_n);
...
%         diff2_g=-2*(1-eta);
        ...
        strain_energy=strain_energy_ductile_iso_M(strain,E);    
        %--------------------------------------
        ...
        %--------------------------------------       
        % Logarithmic fatigue function
        if gamma_gp<=gamma_c
            fdeg=1;
        elseif gamma_gp<=gamma_c*10^(1/k)                
            fdeg=(1-k*log10(gamma_gp/gamma_c))^2;
        else
            fdeg=1e-12;
        end
        %--------------------------------------
