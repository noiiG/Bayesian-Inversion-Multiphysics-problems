...
maxdisp=0.0015/3;
mindisp=0.0015/3;
cycle=[(maxdisp-u_incr):-u_incr:-mindisp (-mindisp+u_incr):u_incr:maxdisp];
cycles = [cycle, repmat(cycle(1:end),1,Nt-1)];
initial=0:u_incr:maxdisp;
loads=diff([initial cycles]);
nstep=numel(loads);
